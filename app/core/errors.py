...
async def http_422_error_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    return JSONResponse(
        {"errors": errors}, status_code=status.HTTP_422_UNPROCESSABLE_ENTITY
    )


async def objectid_invalid_exception_handler(
    request: Request, exec: InvalidId
) -> UJSONResponse:
    return UJSONResponse(
        status_code=status.HTTP_404_NOT_FOUND, content={"detail": "Note not found"}
    )
...
def setup_exception_handlers(app: FastAPI):
    ...
    app.add_exception_handler(
        status.HTTP_422_UNPROCESSABLE_ENTITY, http_422_error_handler
    )
